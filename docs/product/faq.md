---
description: onlyoffice中国版常见问题解答，解决文档下载失败、JWT认证、授权不生效、https部署、子目录部署、复制权限等技术问题。
---
...
- 启动参数需要按照[启动编辑服务](/docs/install/docker) 设置环境变量 `ALLOW_PRIVATE_IP_ADDRESS=true`
- 开档时传递的文件地址是需要在容器内部下载并转换的，所以一定要保证地址在容器内可正常下载
- 不能使用 `127.0.0.1` 或 `localhost` 一类的域名或地址访问服务，容器内会请求到自己内部
- 如果是使用了 `https` 协议的地址，一定要保证证书是正式的而不是自己颁发的，否则也可能失败
- 如果是CDN回源，要保证访问协议与回源协议一致
